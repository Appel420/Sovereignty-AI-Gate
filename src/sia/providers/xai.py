"""
xAI / Grok provider driver.

REAL HTTP DRIVER — requires a valid API key stored at::

    ~/.sia/vault/keys/xai.grok.key

and network connectivity to ``api.x.ai``.
...
_PROVIDER_ID = "xai.grok"
_API_URL = "https://api.x.ai/v1/chat/completions"
_KEY_FILE = Path("~/.sia/vault/keys/xai.grok.key")
...
def _load_api_key(key_file: Path) -> str:
    ...
    if not resolved.exists():
        raise ProviderConfigError(
            _PROVIDER_ID,
            f"[REAL DRIVER — xAI/Grok] API key not found at '{resolved}'. "
            "Store your xAI API key there to enable this provider.",
        )
    key = resolved.read_text(encoding="utf-8").strip()
    if not key:
        raise ProviderConfigError(
            _PROVIDER_ID,
            f"[REAL DRIVER — xAI/Grok] API key file '{resolved}' is empty.",
        )
    return key
...
class GrokProvider(AIProvider):
    """
    REAL HTTP DRIVER — xAI / Grok

    Dispatches authorized context packets to the xAI API.
    API key must exist at ``~/.sia/vault/keys/xai.grok.key``.
    Network connectivity to ``api.x.ai`` is required at call time.
    """
    ...
    def execute(self, packet: AuthorizedContextPacket) -> dict[str, Any]:
        ...
        try:
            ...
            raise ProviderCallError(
                _PROVIDER_ID,
                status=exc.code,
                message=f"[REAL DRIVER — xAI/Grok] HTTP {exc.code}: {exc.reason}",
            ) from exc
        except (urllib.error.URLError, OSError) as exc:
            raise ProviderCallError(
                _PROVIDER_ID,
                message=f"[REAL DRIVER — xAI/Grok] Network error: {exc}",
            ) from exc
