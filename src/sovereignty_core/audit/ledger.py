...
This module owns the in-memory evidence model and its integrity checks. It
does not own persistent storage, private keys, provider authentication, or
transport. Every event is signed by the configured device root and chained to
...
class SCARAttestation:
    ...
    ``signature`` is a detached signature over ``signing_document()``. The
    configured RootOfTrust retains the signing key; this structure never
...
class SCARLedger:
    ...
    All events are signed by the configured device root, even when a provider
    is the actor. A provider therefore never obtains ledger-write authority or
    device signing material.
    """
    ...
    def get_merkle_root(self) -> MerkleTree:
        ...
        return build_merkle_from_scarlog([event.to_dict() for event in self._events])
    ...
    def verify_integrity(self) -> bool:
        """Return whether all event hashes, links, and device signatures verify."""
    ...
    def export_audit_bundle(self) -> dict[str, Any]:
        ...
        The bundle intentionally omits root key material and memory plaintext.
        It includes a signed root attestation when events are present.
        """
        bundle: dict[str, Any] = {
            "format": "SAMA-SCAR-AUDIT-BUNDLE-1",
            "identity_id": self.identity_id,
            "device_id": self.device_id,
            "entries": [event.to_dict() for event in self._events],
        }
        if self._events:
            tree = self.get_merkle_root()
